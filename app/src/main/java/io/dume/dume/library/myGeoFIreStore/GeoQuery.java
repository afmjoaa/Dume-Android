package io.dume.dume.library.myGeoFIreStore;
...
import io.dume.dume.library.myGeoFIreStore.core.GeoHash;
...
public class GeoQuery {
    ...
    private boolean locationIsInQuery(GeoPoint location) {
        return GeoUtils.INSTANCE.distance(new io.dume.dume.library.myGeoFIreStore.GeoLocation(location.getLatitude(), location.getLongitude()), new io.dume.dume.library.myGeoFIreStore.GeoLocation(center.getLatitude(), center.getLongitude())) <= this.radius;
    }
    ...
    private void childAdded(DocumentSnapshot documentSnapshot) {
        GeoPoint location = io.dume.dume.library.myGeoFIreStore.GeoFirestore.Companion.getLocationValue(documentSnapshot);
        if (location != null) {
                ...
                public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                    if (task.isSuccessful()) {
                        ...
                        synchronized (GeoQuery.this) {
                            ...
                            GeoHash hash = (location != null) ? new GeoHash(new io.dume.dume.library.myGeoFIreStore.GeoLocation(location.getLatitude(), location.getLongitude())) : null;
